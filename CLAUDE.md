...
`sectionate.walk.infer_grid_path` step by step to record per-step state for drawing, then
...
(currently ~0.8 MB combined): `docs/**` is not excluded from the sdist by `pyproject.toml`,
...
- **Two axes, and `q`:** the **corner** axis holds `i_c`/`j_c`/`f_c`/`n_c`/`lons_c`/`lats_c`, all 1:1 with each other; the **`sect`** axis holds the velocity faces and their own `lon`/`lat`. They are different lengths on purpose: a seam crossing is written with both spellings of the corner it crosses at, and the step between two spellings of one corner spans no cell, so it emits no face. `uvindices_from_qindices` reports `q` to relate them -- `q[k]` is the index into the caller's corner arrays that face `k` starts at, so face `k` spans corners `q[k]` and `q[k]+1`. It is strictly increasing, skips exactly the steps that carried no face, and is carried as a coordinate on `convergent_transport()` and `extract_tracer()` output. **Input is liberal**: a section traced here, traced on a cell mask, or reloaded from saved indices all give the same faces, however each wrote its seam crossings. **Output is strict**: every returned face is a real velocity face.
- **Known limitation (polar orientation):** a *closed* section with a corner exactly on a geographic pole gets `positive_in` inverted, because orientation comes from the signed area of a stereographic projection and that projection sends one pole to infinity. It affects the four cells around a lat-lon-cap grid's Arctic pole. `convergent_transport` warns; passing `positive_in` as a cell mask avoids the projection entirely. Pinned as a strict xfail in `test_cube_left_grid.py`.
